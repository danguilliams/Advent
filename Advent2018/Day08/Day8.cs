...
namespace AdventOfCode2018
{
    ...
    public class Day8 : Day
    {
        ...
        public override int PuzzleDay => 8;
        private Node Root;
        private int curId;
        ...
        {
            // read the child count and metadata count, then start processing children
            int childCount = vals.Current;
            vals.MoveNext();
            int metadataCount = vals.Current;
            vals.MoveNext();
            Node newNode = new Node(parent, childCount, metadataCount, NextId());
            for (int i = 0; i < childCount; i++)
            {
                newNode.Children[i] = ProcessNode(vals, newNode);
            }
            for (int i = 0; i < metadataCount; i++)
            {
                newNode.Metadata[i] = vals.Current;
                vals.MoveNext();
            }
        ...
        public class Node
        {
            public Node(Node parent, int childCount, int metadataCount, string id)
            {
                Id = id;
                Parent = parent;
