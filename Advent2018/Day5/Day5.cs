...
namespace AdventOfCode2018
{
    ...
    public class Day5 : Day
    {
        ...
        protected override string Part1()
        {
            ...
            return $"Reduced length: {reduced.Length}";
        }

        protected override string Part2()
        {
            Dictionary<char, int> charToLength = new Dictionary<char, int>();
            ...
            char minChar = '-';
            foreach (KeyValuePair<char, int> kvp in charToLength)
            {
                if(kvp.Value < min)
                {
                    min = kvp.Value;
            ...
            return $"Biggest reduction: {minChar} with length {min}";
        }

        private string Reduce(string polymer)
        {
            Stack<char> reduced = new Stack<char>();
            ...
            int reducedLength = reduced.Count;
            StringBuilder sb = new StringBuilder(reduced.Count);
            while(reduced.Count > 0)
            {
                sb.Append(reduced.Pop());
            }
            return sb.ToString();
        }
